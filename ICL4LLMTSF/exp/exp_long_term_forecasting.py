...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def vali(self, vali_data, vali_loader, criterion, is_test=False):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                # # 任务vector实验
                # task_vector = np.load(self.args.root_path + "task_v.npy")
                task_vector = np.load(self.args.root_path + "task_v_ICL.npy")
                # task_vector = np.load("task_v_ICL.npy")
                # task_vector = np.load("temp.npy")
                # task_vector = np.load("task_v.npy")
                # task_vector = np.load("./dataset/weather/task_last.npy")
                # # task_vector = np.load("task_attn_mlp_v.npy")
                # # task_vector = np.load("task_v3_1000.npy")
                ...
                # 均值实验
                # batch_x_mark = batch_x_mark.mean(dim=0, keepdim=True)
                # batch_y_mark = batch_y_mark.mean(dim=0, keepdim=True)
                # batch_x_mark = batch_x_mark.float().to(self.device)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            #     data = torch.cat([batch_x, batch_y[:, -96:, :]], dim=1)
            #     data = data[:, :, 0]
            #     if data_all is None:
            #         data_all = data
            #     else:
